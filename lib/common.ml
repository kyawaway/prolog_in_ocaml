...
let try_parse s = try Some (parse s) with Error -> None
...
let string_of_const c =
  match c with String s -> "StringConst \"" ^ String.escaped s ^ "\""
...
let string_of_dec d =
  match d with
  | Clause (e1, g) ->
      "Clause (" ^ string_of_exp e1 ^ ", " ^ string_of_exp_list g ^ ")"
  | Query g -> "Query (" ^ string_of_exp_list g ^ ")"

let string_of_db db = "[" ^ String.concat "; " (List.map string_of_dec db) ^ "]"
...
let string_of_unify_res s =
  match s with None -> "None" | Some l -> string_of_subs l

let readable_string_of_const c =
  match c with String s -> "\"" ^ String.escaped s ^ "\""
...
let rec readable_string_of_exp e =
  ...
  | TermExp (s, l) ->
      ...
      if List.length l > 0 then
        "(" ^ String.concat ", " (List.map readable_string_of_exp l) ^ ")"
      else ""

let print_db db = print_endline (string_of_db db)
